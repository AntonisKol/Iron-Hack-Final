...
fraud_kpis AS (
    SELECT
        ...
        ROUND(SUM(is_fraud) * 100.0 / COUNT(*), 2) AS fraud_percentage,  -- remove WHERE here
        (SELECT MODE(fraud_type) FROM BANK_TRANSACTIONS WHERE is_fraud = 1) AS top_fraud_type  -- subquery for top fraud type
...
SELECT
    transaction_date,
    COUNT(*)                                    AS total_transactions,   -- transactions per day
    SUM(is_fraud)                               AS fraud_cases,          -- frauds per day
    ROUND(SUM(is_fraud) * 100.0 / COUNT(*), 2) AS fraud_rate,           -- fraud % per day
...
SELECT
    customer_id,
    ROUND(AVG(transaction_amount), 2)          AS avg_transaction_amount,  -- their average transaction
    ROUND(AVG(credit_score), 2)                AS avg_credit_score,        -- their credit score
    MAX(failed_attempts)                        AS max_failed_attempts,     -- worst failed attempts
